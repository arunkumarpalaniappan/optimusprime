...
class SideBar extends Component { 
  constructor(props, context) {
    ...
    this.descChanged = this.descChanged.bind(this);
  }
  ...
  updateOppr() {
    ...
    let backgroundIds = []
    for (i =0 ;i<this.state.bgvalue.length;i++) {
    ...
    if(R.isEmpty(this.opprtitle.value)) {
      ...
      this.setState({addresserr: 'City Name Required',descerr:'',nameerr:'',skillserr:'',backgrounderr:'',applicationerr:'',earliesterr:'',latesterr:'',salaryerr: '',selectionerr:''});
        ...
        appclosedate = Math.floor((appclosedate/1000)/60/60/24);
        if(this.opprtitle.value.length > 100) {
          this.setState({nameerr:'Name should be less than 100 characters',descerr:'',addresserr:'',skillserr:'',backgrounderr:'',applicationerr:'',earliesterr:'',latesterr:'',salaryerr: '',selectionerr:''});
        } else if(appclosedate < -30) {
          this.setState({applicationerr: 'Application Close Date cannot be less than 30 days from current date',descerr:'',addresserr:'',skillserr:'',backgrounderr:'',nameerr:'',earliesterr:'',latesterr:'',salaryerr: '',selectionerr:''});
        } else if(appclosedate > 90) {
          this.setState({applicationerr: 'Application Close Date cannot be more than 90 days from current date',descerr:'',addresserr:'',skillserr:'',backgrounderr:'',nameerr:'',earliesterr:'',latesterr:'',salaryerr: '',selectionerr:''});
        } else {
  ...
  render() {
    let className;
    const {isActive,onDismiss,opprId,opprInfo} = this.props;
    const inputProps = {
            ...
            <div className='overlay-layer-container'>
              <div className='col-sm-12 col-md-12'>
                ...
                <div className='col-sm-12 col-md-12'>
                  ...
                  <Datetime ref={(input) => { this.applicationsclosedate = input; }} defaultValue={new Date(opprInfo.applications_close_date)} />
                  <span className="errclass">{this.state.applicationerr}</span>
                  </div>
                  <div className='col-sm-12 col-md-12 form-group input-group zero-margin'>
                  <h4 className='heading-form-small'>Earlist Start Date</h4>
                  <Datetime ref={(input) => { this.earlieststartdate = input; }} defaultValue={new Date(opprInfo.earliest_start_date)} />
                  <span className="errclass">{this.state.earliesterr}</span>
                  </div>
                  <div className='col-sm-12 col-md-12 form-group input-group zero-margin'>
                  <h4 className='heading-form-small'>Latest End Date</h4>
                  <Datetime ref={(input) => { this.latestenddate = input; }} defaultValue={new Date(opprInfo.latest_end_date)} />
                  <span className="errclass">{this.state.latesterr}</span>
                  </div>
                  <div className='col-sm-12 col-md-12 form-group input-group zero-margin'>
                  <h4 className='heading-form-small'>Description</h4>
                  <input className="form-control" id="description" type="text" value={this.state.descfield} onChange={this.descChanged} ref={(input) => { this.description = input; }} name='description' placeholder="" />
